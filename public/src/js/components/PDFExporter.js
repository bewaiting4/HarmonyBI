		...
		return {
			...
			contactTable: _.map(config.contactTable, function(p, idx) {
				return {
					...
					'通话次数': p.isSpecialNumber && ((p.isSpecialNumber === ENUM.SPECIAL_NUMBER_KEY.NOT) ? ENUM.SPECIAL_NUMBER_MAP[0] : ENUM.SPECIAL_NUMBER_MAP[1]) || "",					
					'通话时长': ENUM.CLOSE_MAP[p.callCount] || "",
...
//					'备注': p.notes || "",				
    ...
    download(config) {
        config.title = config.title || '新疆和田地区皮山县2.11特大暴恐案件分析报告';
